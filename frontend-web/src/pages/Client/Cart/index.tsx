...
import toast from 'react-hot-toast'; // <--- ADICIONADO AQUI
...
import { maskMoney, currencyToNumber } from '../../../utils/masks'; // <--- CORRIGIDO
...
export default function ClientCart() {
  ...
  // Garante que é booleano (evita erro de null)
  ...
  function validateOrder() {
    if (isStaff) { 
      toast.error('Membros da equipe não podem realizar pedidos.');
      return false; 
    }
    ...
    if (paymentType === 'OFFLINE' && paymentMethod === 'MONEY') {
      ...
      if (!changeValue || changeValue < totalFinal) {
        setErrorMsg('⚠️ O valor para troco deve ser maior que o total do pedido.');
  ...
  return (
    <div className="max-w-2xl mx-auto space-y-6 pb-40">
      ...
      {isStaff && (
        <div className="bg-red-100 border-l-4 border-red-500 text-red-700 p-4 rounded shadow-sm flex items-start gap-3">
          ...
          <div>
            ...
            <p className="text-sm">Você está logado como membro da equipe. A realização de pedidos está bloqueada neste perfil.</p>
          </div>
        </div>
      )}
      ...
      <div className="bg-white p-5 rounded-2xl shadow-sm border border-gray-100 space-y-4">
        ...
        {/* Desabilita se for Staff */}
        <div className={`flex p-1 bg-gray-100 rounded-xl mb-4 ${isStaff ? 'opacity-50 pointer-events-none' : ''}`}>
               ...
               paymentStep === 'PIX_WAIT' && pixData ? (
                 ...
                 <div className="text-center py-6">
                   ...
                   <button 
                      ...
                      className="bg-green-600 text-white font-bold px-6 py-3 rounded-xl hover:bg-green-700 transition-all flex items-center gap-2 mx-auto disabled:opacity-50 disabled:cursor-not-allowed"
                   >
          ...
          <div className="animate-in fade-in space-y-4">
             <div>
                ...
                <select 
                  ...
                  disabled={isStaff}
                  className="w-full p-3 bg-gray-50 border border-gray-200 rounded-xl outline-none focus:border-orange-500 disabled:opacity-50"
                >
             ...
             {paymentMethod === 'MONEY' && (
               <div className="bg-orange-50 p-4 rounded-xl border border-orange-100 animate-in slide-in-from-top-2">
                 ...
                 <input 
                   ...
                   disabled={isStaff}
                   placeholder="R$ 0,00"
                   className="w-full p-3 bg-white border border-gray-200 rounded-lg font-bold text-lg text-gray-800 focus:ring-2 focus:ring-orange-500 outline-none"
                 />
                 <p className="text-xs text-gray-500 mt-2">
                   O entregador levará o troco baseado neste valor. O total do pedido é <b>{totalFinal.toLocaleString('pt-BR', { style: 'currency', currency: 'BRL' })}</b>.
                 </p>
               </div>
             )}
          </div>
