...
export default function AdminDashboard() {
  ...
  function changeTab(tab: 'overview' | 'orders' | 'products' | 'users') {
    setActiveTab(tab);
    setIsSidebarOpen(false);
  }
        ...
        return (
          <ScrollView className="flex-1 px-4 pt-6" showsVerticalScrollIndicator={false}>
            ...
            >
              ...
              <Feather name="power" size={140} color="rgba(255,255,255,0.1)" style={{ position: 'absolute', right: -30, bottom: -40, transform: [{ rotate: '20deg' }] }} />
            </TouchableOpacity>

            <View className="flex-row flex-wrap justify-between mb-2">
              <KpiCard title="Faturamento" value={formatPrice(summary.revenue)} icon="dollar-sign" colorClass="bg-emerald-500" />
              <KpiCard title="Pedidos Hoje" value={summary.totalOrders} icon="shopping-bag" colorClass="bg-blue-500" />
              <KpiCard title="Ticket Médio" value={formatPrice(summary.avgTicket)} icon="trending-up" colorClass="bg-violet-500" />
              <KpiCard title="Fila (Ativos)" value={summary.activeOrders} icon="clock" colorClass="bg-orange-500" />
            </View>
  ...
  return (
    <SafeAreaView className="flex-1 bg-gray-50 dark:bg-slate-900 pt-8">
      
      {/* HEADER NAVBAR */}
      <View className="px-6 py-4 flex-row items-center justify-between bg-white dark:bg-slate-800 border-b border-gray-200 dark:border-slate-700 z-10">
        <View className="flex-row items-center">
          <TouchableOpacity onPress={() => setIsSidebarOpen(true)} className="p-2 mr-3 bg-gray-100 dark:bg-slate-700 rounded-lg active:scale-95">
            <Feather name="menu" size={24} color={activeTheme === 'dark' ? '#f8fafc' : '#334155'} />
          </TouchableOpacity>
          <Text className="text-xl font-bold text-slate-900 dark:text-white capitalize">
            {activeTab === 'overview' ? 'Painel' : activeTab === 'orders' ? 'Pedidos' : activeTab === 'products' ? 'Cardápio' : 'Equipe'}
          </Text>
        </View>
        <TouchableOpacity onPress={cycleTheme} className="p-2.5 bg-gray-100 dark:bg-slate-700 rounded-xl active:scale-95">
      ...
      <Modal visible={isSidebarOpen} transparent animationType="fade">
        <View className="flex-1 flex-row">
          <TouchableOpacity className="absolute inset-0 bg-black/60 z-0" activeOpacity={1} onPress={() => setIsSidebarOpen(false)} />
          ...
          <View className="w-64 h-full bg-white dark:bg-slate-800 shadow-2xl z-10 flex-col py-12 px-6">
            <View className="flex-row justify-between items-center mb-8 border-b border-gray-100 dark:border-slate-700 pb-4">
              <View>
                ...
                <Text className="text-xs text-gray-500 dark:text-gray-400 mt-1">Olá, {user?.name?.split(' ')[0]}</Text>
              </View>
            ...
            <ScrollView className="flex-1" showsVerticalScrollIndicator={false}>
              <TouchableOpacity onPress={() => changeTab('overview')} className={`flex-row items-center py-4 px-4 rounded-xl mb-2 ${activeTab === 'overview' ? 'bg-orange-50 dark:bg-orange-900/20' : ''}`}>
                <Feather name="layout" size={20} color={activeTab === 'overview' ? '#ea580c' : (activeTheme === 'dark' ? '#cbd5e1' : '#475569')} />
                <Text className={`ml-3 font-bold ${activeTab === 'overview' ? 'text-orange-600 dark:text-orange-500' : 'text-slate-600 dark:text-slate-300'}`}>Dashboard</Text>
              </TouchableOpacity>
              
              <TouchableOpacity onPress={() => changeTab('orders')} className={`flex-row items-center py-4 px-4 rounded-xl mb-2 ${activeTab === 'orders' ? 'bg-orange-50 dark:bg-orange-900/20' : ''}`}>
                <Feather name="shopping-bag" size={20} color={activeTab === 'orders' ? '#ea580c' : (activeTheme === 'dark' ? '#cbd5e1' : '#475569')} />
                <Text className={`ml-3 font-bold ${activeTab === 'orders' ? 'text-orange-600 dark:text-orange-500' : 'text-slate-600 dark:text-slate-300'}`}>Pedidos</Text>
              </TouchableOpacity>
              
              <TouchableOpacity onPress={() => changeTab('products')} className={`flex-row items-center py-4 px-4 rounded-xl mb-2 ${activeTab === 'products' ? 'bg-orange-50 dark:bg-orange-900/20' : ''}`}>
                <Feather name="box" size={20} color={activeTab === 'products' ? '#ea580c' : (activeTheme === 'dark' ? '#cbd5e1' : '#475569')} />
                <Text className={`ml-3 font-bold ${activeTab === 'products' ? 'text-orange-600 dark:text-orange-500' : 'text-slate-600 dark:text-slate-300'}`}>Produtos</Text>
              </TouchableOpacity>

              <TouchableOpacity onPress={() => changeTab('users')} className={`flex-row items-center py-4 px-4 rounded-xl mb-2 ${activeTab === 'users' ? 'bg-orange-50 dark:bg-orange-900/20' : ''}`}>
                <Feather name="users" size={20} color={activeTab === 'users' ? '#ea580c' : (activeTheme === 'dark' ? '#cbd5e1' : '#475569')} />
                <Text className={`ml-3 font-bold ${activeTab === 'users' ? 'text-orange-600 dark:text-orange-500' : 'text-slate-600 dark:text-slate-300'}`}>Usuários</Text>
              </TouchableOpacity>
            </ScrollView>

            <View className="border-t border-gray-100 dark:border-slate-700 pt-4 mt-auto">
              <TouchableOpacity onPress={handleSignOut} className="flex-row items-center py-3 px-4 rounded-xl hover:bg-red-50 active:bg-red-100 dark:active:bg-red-900/30">
